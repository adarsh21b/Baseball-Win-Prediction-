...
SELECT
    ...
    , CASE WHEN (pc.atBat - pc.Strikeout - pc.Home_Run + pc.Sac_Fly) != 0 THEN ((pc.Hit - pc.Home_Run) / (pc.atBat - pc.Strikeout - pc.Home_Run + pc.Sac_Fly)) ELSE 0 END AS BABIP
...
CREATE INDEX IF NOT EXISTS game_id_idx_new ON pitcher_temp (game_id);
CREATE INDEX IF NOT EXISTS  game_date_idx_new ON pitcher_temp (game_date);
CREATE INDEX IF NOT EXISTS team_id_game_idx_new ON pitcher_temp (team_id);
CREATE INDEX IF NOT EXISTS idx_pitcher_temp_team_game_date ON pitcher_temp (team_id, game_date);
CREATE INDEX IF NOT EXISTS idx_pitcher_temp_game_id_team_id_pitcher ON pitcher_temp (game_id, team_id, pitcher);
...
SELECT
    ...
    , CASE WHEN (SUM(pc.atBat) - SUM(pc.Strikeout) - SUM(pc.Home_Run) + SUM(pc.Sac_Fly)) != 0 THEN (SUM(pc.Hit) - SUM(pc.Home_Run) / (SUM(pc.atBat) - SUM(pc.Strikeout) - SUM(pc.Home_Run) + SUM(pc.Sac_Fly))) ELSE 0 END AS BABIP_100
...
SELECT
    ...
    , (pt_home.BABIP_100 - pt_away.BABIP_100 ) AS BABIP_diff
