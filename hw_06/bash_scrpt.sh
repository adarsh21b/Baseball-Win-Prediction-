...
then
  ...
  mysql -u root -p9501 -h mariadb_db  -e "select * from Rolling_AVG_100_day;" < records.csv
  echo 'records recorded in records.csv'

else
  echo 'the database does not exists. we are gonna load the database first'
  mysql -u root -p 9501 -h mariadb_db  baseball < baseball.sql
  echo 'baseball db loaded. gonna run the hw_02 file'
  mysql -u root -p 9501 -h mariadb_db  baseball < hw_02.sql
  echo 'records recorded in records.csv'
